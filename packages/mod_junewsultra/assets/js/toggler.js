...
if (typeof (window['nnToggler']) == "undefined") {
    ...
    var nnToggler = new Class({
        ...
        initTogglers: function () {
            ...
            $each(this.togglers, function (toggler) {
                if (toggler.nofx) {
                    toggler.fx.slide = new Fx.Slide(toggler, {
                        'duration': 1,
                        'mode': toggler.mode,
                        onComplete: function () {
                            self.completeSlide(toggler)
                        }
                    })
                } else {
                    toggler.fx.slide = new Fx.Slide(toggler, {
                    ...
                    toggler.fx.fade = new Fx.Morph(toggler, {
                        'duration': 500
                    })
                }
            });
        ...
        autoHeightDivs: function () {
            $each(document.getElements('div.col div, div.fltrt div'), function (el) {
                if (el.getStyle('height') != '0px' && !el.hasClass('input') && !el.hasClass('nn_hr') && !el.hasClass('textarea_handle') && el.id.indexOf('gk_') === -1 && el.className.indexOf('gk_') === -1 && el.className.indexOf('switcher-') === -1) {
